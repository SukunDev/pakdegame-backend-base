    ...
    @php
        ...
        @for ($i = $startCount; $i <= $endCount; $i++)
            <a href="{{ $paginator->url($i) }}"
                class="px-3 md:px-4 py-2 font-bold rounded-md text-xs md:text-base border border-slate-700 {{ $paginator->currentPage() == $i ? 'pointer-events-none cursor-default bg-slate-700 text-white' : 'text-gray-600 bg-gray-50 hover:bg-slate-700 hover:text-white transition duration-300' }}">
                {{ $i }}
            </a>
        @endfor
